...
from config import MODEL_CONFIG, DEEPSEEK_CONFIG, QWEN_CONFIG, DEFAULT_MODEL, APP_CONFIG
...
# 对话存储
conversations = {}  # 格式: {conversation_id: {'messages': [...], 'last_updated': timestamp}}
conversation_counter = 1

# 调用模型（支持流式输出）
def get_model_response(prompt, history=[], stream=False, model=None):
    try:
        # 确定使用的模型
...
def chat():
    ...
    conversations[conversation_id]['last_updated'] = time.time()
    
    # 获取模型响应
    response = get_model_response(message, conversations[conversation_id], stream, model)
    ...
    if stream:
        if DEFAULT_MODEL == "deepseek":
            ...
            def generate():
                ...
                # 添加完整响应到对话历史
                conversations[conversation_id]['messages'].append({'role': 'assistant', 'content': full_response})
                conversations[conversation_id]['last_updated'] = time.time()
